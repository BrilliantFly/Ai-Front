<template>
  <div class="plan-slogan-container">
    ...
    <a-card :bordered="false" class="search-form">
      <a-form :model="queryParams" layout="inline">
        <a-form-item label="内容关键词">
          <a-input v-model:value="queryParams.content" placeholder="请输入内容关键词" allow-clear @pressEnter="fetchData" />
        </a-form-item>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'timeRange'">
            <span v-if="!record.startTime && !record.endTime" class="text-muted">不限</span>
            <template v-else>
    ...
    <!-- 新增/编辑弹窗 -->
    <a-modal v-model:open="modalVisible" :title="modalTitle" @ok="handleOk" :confirmLoading="confirmLoading" width="640px">
      <a-form :model="formData" :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }">
        <a-form-item label="标语内容" required>
          <a-textarea v-model:value="formData.content" placeholder="请输入标语内容，如：努力是光，坚持是路" :rows="3" :maxlength="200" show-count />
        </a-form-item>
        <a-form-item label="表情符号">
          <a-input v-model:value="formData.emoji" placeholder="可输入表情/符号，如：✨ 🚀 🌱，也可直接粘贴" allow-clear />
        ...
        <a-form-item label="生效时间">
          <a-date-picker
            ...
            :allow-empty="true"
          />
          <div class="form-tip">留空表示一直生效；设置时间段后，到达开始时间自动切换为该标语</div>
        </a-form-item>
        <a-form-item label="排序">
          <a-input-number v-model:value="formData.sort" :min="0" :max="999" style="width: 120px" />
          <div class="form-tip">数字越小越靠前，多个同时生效时按排序取第一条</div>
        </a-form-item>
        <a-form-item label="状态">
...
const timeRange = ref<[number | null, number | null] | null>(null)
...
const columns = [
  ...
  { title: '生效时间', key: 'timeRange', width: 200 },
...
const handleEdit = (record: any) => {
  ...
  if (record.startTime || record.endTime) {
    timeRange.value = [record.startTime ?? null, record.endTime ?? null]
  } else {
    timeRange.value = null
  }
  modalVisible.value = true
...
const handleOk = async () => {
  ...
  // 组装时间范围
  formData.startTime = timeRange.value?.[0] ?? undefined
  formData.endTime = timeRange.value?.[1] ?? undefined
